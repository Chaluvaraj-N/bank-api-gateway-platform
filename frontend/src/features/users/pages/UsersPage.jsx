...
const rows = [
  { name: "Ava Johnson", email: "ava.johnson@corebanking.com", role: "Admin", status: "Active" },
  { name: "Noah Smith", email: "noah.smith@partnerapi.com", role: "Operator", status: "Active" },
  { name: "Mia Chen", email: "mia.chen@riskops.com", role: "Auditor", status: "Suspended" },
  { name: "Liam Patel", email: "liam.patel@corebanking.com", role: "Approver", status: "Active" },
];
  ...
  return (
    <EnterpriseLayout rightPanel={<RightPanel analyticsOverview={analyticsOverview} systemHealth={systemHealth} />}>
      <div className="p-5 md:p-6">
        <div className="text-slate-900 text-xl font-bold">Users</div>
        <div className="text-slate-500 text-sm mt-1">Manage administrator accounts, access, and status.</div>

        <div className="mt-5 bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div className="font-semibold text-slate-900">User Directory</div>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="border-t border-slate-100">
                    <td className="px-3 py-2 text-slate-700 whitespace-nowrap font-semibold">{r.name}</td>
                    <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{r.email}</td>
                    <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{r.role}</td>
